...
uses
  ...
  StdCtrls, Windows, LazUTF8, LazFileUtils;
...
begin
  if GetFileVersion((FileName), VerInfo) then
    Result := Format('%u.%u.%u.%u', [HiWord(VerInfo.dwFileVersionMS),
...
begin
  ...
  begin
    ...
    Sleep(1000);
    if (FileUtil.CopyFile(updatePATH + 'updates\EWLog.exe', 'EWLog.exe', True, True)) and
      (FileUtil.CopyFile(updatePATH + 'updates\serviceLOG.db', updatePATH + 'serviceLOG.db',
      True, True)) then
    begin
