  ...
  begin
    ...
    begin
      Application.MessageBox(
        'Пожалуйста укажите файл для экспорта!', 'Внимание!',
    ...
    if CheckBox1.Checked = True then
      ...
      Application.MessageBox('Не выбран метод экспорта!',
        'Внимание!',
        mb_ok + mb_IconWarning);
  end;
end;
...
begin
  ...
  if IOresult <> 0 then
  begin
    Application.MessageBox(PChar('Ошибка открытия файла : ' +
      IntToStr(IOResult)),
  ...
  begin
    if DefaultDB = 'MySQL' then
      ...
      Q1.SQL.Text :=
        ...
        'strftime(' + QuotedStr('%Y-%m-%d') + ',QSODate) BETWEEN ' +
  ...
  begin
    ...
    begin
      Q1.SQL.Text := 'SELECT * FROM ' + LogTable +
...
begin
  ...
  if DefaultDB = 'MySQL' then
      ...
      Q1.SQL.Text :=
        'SELECT * FROM ' + LogTable + ' WHERE ' +
        'strftime(' + QuotedStr('%Y-%m-%d') + ',QSODate) >= ' +
